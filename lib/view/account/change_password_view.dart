...
class _ChangePasswrodViewState extends State<ChangePasswrodView> {
  ...
  initPref() async {
    ...
    if (userMap != null) {
      setState(() {
        playerProfile = UserProfile.fromJson(jsonDecode(userMap));
      });
    }
  }
  ...
  onOk() async {
    ...
    req.passwordOld = password.text;
    req.phoneNumber = playerProfile!.phoneNumber;
    if (mounted) showProcess(context);

    ResponseObject res = await con.changePassword(req);
    if (mounted) Navigator.pop(context);
    if (res.code == "00") {
      showMessage("Thay đổi mật khẩu thành công", "00", 5);
    } else {
